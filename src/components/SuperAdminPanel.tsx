          ...
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200 self-start md:self-center">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                activeSubTab === 'tenants' ? 'bg-blue-600 text-white shadow shadow-blue-250' : 'text-slate-500 hover:text-slate-800 hover:bg-white/40'
              }`}
            >
              <Users className="w-3.5 h-3.5 inline mr-1" /> Assinantes ({tenants.length})
            </button>
            <button
              onClick={() => setActiveSubTab('coupons')}
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                activeSubTab === 'coupons' ? 'bg-blue-600 text-white shadow shadow-blue-250' : 'text-slate-500 hover:text-slate-800 hover:bg-white/40'
              }`}
            >
              <Ticket className="w-3.5 h-3.5 inline mr-1" /> Cupons ({coupons.length})
            </button>
            <button
              onClick={() => setActiveSubTab('suporte')}
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                activeSubTab === 'suporte' ? 'bg-blue-600 text-white shadow shadow-blue-250' : 'text-slate-500 hover:text-slate-800 hover:bg-white/40'
              }`}
            >
              <HeartHandshake className="w-3.5 h-3.5 inline mr-1" /> Suporte ({supportTickets.filter(t => t.status === 'open').length} pendentes)
            </button>
            <button
              onClick={() => setActiveSubTab('logs')}
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-all cursor-pointer ${
                activeSubTab === 'logs' ? 'bg-blue-600 text-white shadow shadow-blue-250' : 'text-slate-500 hover:text-slate-800 hover:bg-white/40'
              }`}
            >
              <ShieldCheck className="w-3.5 h-3.5 inline mr-1" /> Rastreio e LGPD
            </button>
          </div>
        </div>
      </div>
