...
export default function WaitlistModal({ tenantId, tenantName, tenantSlug, professionals, onClose }: Props) {
  ...
  const [autoSending, setAutoSending] = useState<Record<string, boolean>>({});
  const [autoFailed,  setAutoFailed]  = useState<Set<string>>(new Set());
  const [authToken,   setAuthToken]   = useState('');

  // Evita re-tentativa automática para entradas já processadas nesta sessão
  ...
  // Envio automático ao carregar — uma vez por entrada por sessão
  useEffect(() => {
    if (loading || !authToken || aguardando.length === 0) return;

    aguardando.forEach((entry, idx) => {
      if (autoAttempted.current.has(entry.id)) return;
      autoAttempted.current.add(entry.id);

      setAutoSending(s => ({ ...s, [entry.id]: true }));
      setTimeout(async () => {
        try {
          ...
          if (result === 'sent') {
            ...
            setAutoFailed(prev => new Set([...prev, entry.id]));
          }
        } catch {
          setAutoFailed(prev => new Set([...prev, entry.id]));
        }
        setAutoSending(s => ({ ...s, [entry.id]: false }));
      }, idx * 3000);
    });
  ...
  const sendOne = async (entry: WaitlistEntry) => {
    ...
    try {
      ...
      if (result === 'sent') {
        ...
        setAutoFailed(prev => { const n = new Set(prev); n.delete(entry.id); return n; });
      }
    } catch {}
  ...
  const EntryRow = ({ e }: { e: WaitlistEntry }) => {
    const isAutoSending = autoSending[e.id];
    const hasFailed     = autoFailed.has(e.id);
    ...
    return (
      <div style={{ padding: '12px 16px', background: '#FFFFFF', border: `1px solid ${hasFailed ? '#FCA5A5' : '#E2E8F0'}`, borderRadius: 12, display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' as const }}>
            ...
            {isAutoSending && (
              <span style={{ fontSize: 10, color: '#6B7280', display: 'flex', alignItems: 'center', gap: 3 }}>
                <RefreshCw size={9} style={{ animation: 'spin 1s linear infinite' }} /> enviando…
              </span>
            )}
            {hasFailed && !isAutoSending && (
        ...
        {/* Actions — botão manual só aparece se envio automático falhou */}
        <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
          {hasFailed && (
            <button onClick={() => sendOne(e)} disabled={isSending || !authToken}
              title="Envio automático falhou — tentar novamente"
      ...
      <div onClick={e => e.stopPropagation()}
        ...
        <div style={{ padding: '10px 20px', borderTop: '1px solid #F1F5F9', flexShrink: 0, background: '#F8FAFC' }}>
          <p style={{ fontSize: 11, color: '#9CA3AF', margin: 0, textAlign: 'center' }}>
            Envio automático em ordem de chegada · 3s entre cada envio · botão manual aparece apenas se o envio falhar
          </p>
        </div>
      </div>
    </div>
